    ...
    if (leng > 0):
        if (leng < 3):
            ...
            if (high > sell[sell_flag]):
                ...
                print("卖出价格",sell[sell_flag],"卖出数量",2,"卖出日期索引",i,"剩余",buy)
                sell_flag += 1
                sell.append(0)  # 每次要append，否则会超出内存
                sell_num.append(0)
    else:
...
for i in range(startrow,endrow):    #range(开始索引，结束索引）
    ...
    if(len(buy)<2):
        ...
        ###策略D：超高波动  美诺华（下跌趋势）
        # newbuy=buy_gap*(1+(sp_rate-2)*1.5+(sp_rate-3)*2)       #mutishape1，夏普越高，gap越大
        # newsell = sell_gap * (1.2+(sp_rate-2)*1.5)
        newbuy=buy_gap*(1+(sp_rate-2)*2.5)       #mutishape1，夏普越高，gap越大
        newsell = sell_gap * (1.2+(sp_rate-2)*1.5)
...
print(stockname,"策略：乘法递进T"," 买入T",buy_gap,"   卖出T",sell_gap)
print("买入总计",buy_sum)
